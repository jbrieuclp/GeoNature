...
def filtered_metadata_query(info_role, args):
    ...
    query = (
        ...
        .options(
            joinedload(t_af.t_datasets.of_type(t_dts))
            .joinedload(t_dts.cor_dataset_actor)
    ...
    if organisme is not None:
        query = query.filter(BibOrganismes.id_organisme == 1)
    if role is not None:
